        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                films.forEach(film => {
                    ...
                    if (ratingString) {
                        // Count actual star characters instead of string length
                        // to handle Unicode correctly (string.length can give wrong count)
                        const fullStars = (ratingString.match(/★/g) || []).length;
                        const hasHalfStar = ratingString.includes('½');
                        ratingValue = fullStars + (hasHalfStar ? 0.5 : 0);
                    }
